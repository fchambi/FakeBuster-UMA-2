...
contract PredictionMarketNews is OptimisticOracleV3CallbackRecipientInterface {
    ...
    struct Market {
        ...
        bytes headline; // Headline of the market.
        bytes url; // URL of the market.
    }
    ...
    function initializeMarket(
        ...
        string memory headline, // Description of the headline.
        string memory url, // URL de la noticia.
        uint256 reward, // Reward available for asserting true market outcome.
        uint256 requiredBond // Expected bond to assert market outcome (OOv3 can require higher bond).
        ...
        require(bytes(headline).length > 0, "Empty headline");
        require(bytes(url).length > 0, "Empty url");
        marketId = keccak256(abi.encode(block.number, description, headline, url));
    ...
    function assertMarket(bytes32 marketId, string memory assertedOutcome) public returns (bytes32 assertionId) {
        ...
        bytes memory claim = _composeClaim(assertedOutcome, market.description, market.headline, market.url);
    ...
    function _composeClaim(string memory outcome, bytes memory description, bytes memory headline, bytes memory url) internal view returns (bytes memory) {
        return
            abi.encodePacked(
